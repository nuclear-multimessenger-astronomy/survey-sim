...
We use the [ZTFReST](https://github.com/growth-astro/ztfrest) detection criteria, which require a fast-fading transient detected at least twice with ≥3 hour separation within 14 days.
...
boom_files = sorted(glob.glob(f"{boom_dir}/ztf_2018*.h5")
                  ...
                  + glob.glob(f"{boom_dir}/ztf_202101.h5")
                  + glob.glob(f"{boom_dir}/ztf_202102.h5")
                  + glob.glob(f"{boom_dir}/ztf_202103.h5"))
...
!!! note "GPU acceleration"
    The Bu2026 model supports GPU batch evaluation via JAX. On a GPU node, 1M transients complete in ~6 minutes. On CPU, use a smaller sample (e.g., 100K).
...
| Model | \(N_\mathrm{sim}\) | \(N_\mathrm{det}\) | Efficiency | Runtime |
|-------|---------------------|---------------------|-----------|---------|
| Bu2026 (fixed incl) | 1,000,000 | ~26,500 | 2.65% | ~6 min (GPU) |
| Metzger BB | 1,000,000 | 66 | 0.0066% | ~45 s |

The ~400× difference arises because:

1. **Color coverage**: Bu2026 produces bright red/infrared emission from the lanthanide-rich dynamical ejecta that persists for days, while the Metzger 1-zone model fades rapidly in all bands
2. **Temperature structure**: The two-component Bu2026 model has a hot blue component (wind) and a cooler red component (dynamical) simultaneously, whereas the single-zone Metzger model has one temperature
3. **Late-time behaviour**: The Metzger model drops below detection threshold by day 2–3 at \(z \gtrsim 0.05\), while Bu2026 remains detectable in r/i for 4–5 days
...
| Quantity | Bu2026 | Metzger BB |
|----------|--------|------------|
| \(f_\mathrm{sky}\) | 0.47 | 0.47 |
| \(V_\mathrm{eff}\) | 5.17 Gpc³ | 5.17 Gpc³ |
| Duration | 3.09 yr | 3.09 yr |
| Efficiency | 2.65% | 0.0066% |
| \(VT_\mathrm{eff}\) | 0.423 Gpc³ yr | 0.00105 Gpc³ yr |
| \(R_\mathrm{upper}\) (90%) | **5 Gpc⁻³ yr⁻¹** | 2,185 Gpc⁻³ yr⁻¹ |
| \(R_\mathrm{upper}\) (95%) | 7 Gpc⁻³ yr⁻¹ | 2,842 Gpc⁻³ yr⁻¹ |

!!! info "Interpretation"
    The Bu2026 model gives a much more constraining rate limit because it predicts brighter, longer-lasting emission. The true detection efficiency lies somewhere between the two, depending on how well the kilonova matches each model. The Metzger BB result should be considered a conservative lower bound on efficiency.

---

## Varying the viewing angle

Real kilonovae are observed from random orientations. The Bu2026 model supports uniform inclination sampling:
...
pop = FixedBu2026KilonovaPopulation(
    ...
    v_ej_wind=0.1,
    ye_wind=0.35,
    inclination_em=0.0,         # ignored when vary_inclination=True
    rate=1000.0,
    z_max=0.3,
...
This typically reduces the detection efficiency by a factor of ~2–3 compared to the face-on case, since edge-on viewing angles produce fainter, redder emission.

---

## Full script

A complete end-to-end script for the Metzger BB analysis:

```python
#!/usr/bin/env python
"""ZTF kilonova detection efficiency with Metzger BB model."""
import glob
import math
from survey_sim import (
    SurveyStore,
...
# Survey
boom_files = sorted(glob.glob("/path/to/ztf_boom/ztf_201*.h5")
                  + glob.glob("/path/to/ztf_boom/ztf_20200*.h5")
                  + glob.glob("/path/to/ztf_boom/ztf_20201*.h5")
                  + glob.glob("/path/to/ztf_boom/ztf_20210[123].h5"))
survey = SurveyStore.from_ztf_boom(boom_files, nside=64)

# Population (AT2017gfo best-fit)
pop = FixedMetzgerKilonovaPopulation(
